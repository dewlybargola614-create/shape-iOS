...
// Synthesize WAV sound effect samples
function generateToneSamples(freq, duration, type = 'sine', sampleRate = 22050) {
...
function precalculateAudioAssets() {
  ...
  // 1. Pop Sound (0.06s)
  const popSamples = generateToneSamples(800, 0.06, 'sine', sampleRate);
  audioAssets.pop = createWavDataUri(popSamples, sampleRate);

  // 2. Lift Sound (0.15s)
  const liftSamples = generateToneSamples(500, 0.15, 'triangle', sampleRate);
  audioAssets.lift = createWavDataUri(liftSamples, sampleRate);

  // 3. Shuffle Sound (0.05s woodblock pop)
  const shuffleSamples = generateToneSamples(420, 0.05, 'triangle', sampleRate);
  audioAssets.shuffle = createWavDataUri(shuffleSamples, sampleRate);

  // 4. Correct Sound Fanfare (0.4s 4-tone sweep)
  const correctLen = Math.floor(sampleRate * 0.45);
  const correctSamples = new Float32Array(correctLen);
  const notes = [523.25, 659.25, 783.99, 1046.50];
  const noteDuration = 0.1;
  for (let i = 0; i < correctLen; i++) {
    const t = i / sampleRate;
    const noteIdx = Math.min(3, Math.floor(t / noteDuration));
    const freq = notes[noteIdx];
    const localT = t - noteIdx * noteDuration;
    const wave = Math.sin(2 * Math.PI * freq * t);
    const env = Math.exp(-localT * 10);
    correctSamples[i] = wave * env * 0.5;
  }
  audioAssets.correct = createWavDataUri(correctSamples, sampleRate);

  // 5. Wrong Buzz (0.35s)
  const wrongLen = Math.floor(sampleRate * 0.35);
  const wrongSamples = new Float32Array(wrongLen);
  for (let i = 0; i < wrongLen; i++) {
    const t = i / sampleRate;
    const freq = t < 0.15 ? 280 : 200;
    const wave = 2 * (t * freq - Math.floor(t * freq + 0.5));
    const env = Math.exp(-t * 4);
    wrongSamples[i] = wave * env * 0.4;
  }
  audioAssets.wrong = createWavDataUri(wrongSamples, sampleRate);

  // 6. BGM Loop Track (3.2 seconds cheerful melody)
  const bgmLen = Math.floor(sampleRate * 3.2);
  const bgmSamples = new Float32Array(bgmLen);
  const bgmNotes = [261.63, 329.63, 392.00, 523.25, 392.00, 329.63, 261.63, 392.00];
  const stepDur = 0.4;
  for (let i = 0; i < bgmLen; i++) {
    const t = i / sampleRate;
    const noteIdx = Math.floor(t / stepDur) % bgmNotes.length;
    const freq = bgmNotes[noteIdx];
    const localT = t % stepDur;
    const wave = Math.sin(2 * Math.PI * freq * t);
    const env = Math.exp(-localT * 3);
    bgmSamples[i] = wave * env * 0.12; // Gentle low volume
  }
  audioAssets.bgm = createWavDataUri(bgmSamples, sampleRate);
}
...
function playShuffleSound() { playDataUriSound('shuffle'); }
