...
// Cut the music out entirely while the cups are actively shuffling (a
// partial volume dip was too subtle under the pop sounds), then resume
// it once it's time to guess.
function duckBGM() {
  if (!bgmAudio) return;
  fadeBgmVolume(0, 150);
  setTimeout(() => {
...
function unduckBGM() {
  ...
  fadeBgmVolume(BGM_NORMAL_VOLUME, 300);
}
